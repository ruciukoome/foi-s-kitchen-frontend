...
export const defaultLabels: Record<ServiceCategory, string> = {
  corporate: "Corporate Catering",
  weddings: "Weddings & Private Events",
  "meal-prep": "Meal Prep Plans",
};

/** Dedicated, crawlable URL for each service category. */
  ...
  return (
    <section className="container-page pb-12 md:pb-20">
      <div className="flex flex-wrap items-baseline gap-x-1 gap-y-1 px-0.5 pt-[26px] pb-[14px]">
        <h1 className="font-serif-eyebrow">{label} in Nairobi</h1>
      ...
      <div className="animate-fade-up overflow-hidden rounded-2xl border border-border bg-card shadow-card">
        <nav aria-label="Service categories" className="grid grid-cols-3 border-b border-border">
          {serviceCategories.map((id) => {
            const service = services.data?.find((s) => s.category === id);
            const selected = id === category;
            const name = service?.name ?? defaultLabels[id];
            return (
              <Link
                key={id}
                to={servicePaths[id]}
                ...
                className={cn(
                  "group relative isolate block min-h-[112px] overflow-hidden border-r border-border p-3 text-left transition-all duration-300 ease-out last:border-r-0 sm:min-h-[148px] sm:p-5",
              ...
              >
                {service && (
                  <img
                    ...
                    width={1024}
                    height={640}
                    className="absolute inset-0 -z-20 h-full w-full object-cover transition-transform duration-300 ease-out group-hover:scale-[1.04]"
                  />
                )}
                <span
                  className={cn(
                    "absolute inset-0 -z-10 bg-foreground transition-opacity duration-300 ease-out",
                ...
                <span className="flex h-full flex-col justify-between gap-3">
                  <span
                    className={cn(
                      "h-2.5 w-2.5 rounded-full border border-primary-foreground",
                      selected && "bg-primary",
                    )}
                    aria-hidden="true"
                  />
        ...
        <div key={category} className="animate-swap-in p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col gap-4 border-t border-border bg-secondary/50 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <a
              href={waLink(intro.whatsapp)}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={`Ask about ${label} on WhatsApp`}
              className="label-caps inline-flex min-h-[48px] items-center justify-center rounded-full bg-whatsapp px-5 text-whatsapp-foreground transition-all duration-200 ease-out hover:scale-[1.02] active:scale-[0.97]"
            >
              Ask a question
            </a>
            <Link
              to="/order"
              className="label-caps inline-flex min-h-[48px] items-center justify-center rounded-full bg-primary px-5 text-primary-foreground transition-all duration-200 ease-out hover:bg-primary-deep hover:scale-[1.02] active:scale-[0.97]"
            >
              Order online
            </Link>
          </div>
...
function ServiceTier({ tier, delay }: { tier: ServiceTierRow; delay: number }) {
  ...
  return (
    <article
      className="animate-fade-up group flex flex-col overflow-hidden rounded-2xl border border-border bg-background"
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="aspect-[16/10] overflow-hidden bg-secondary/60">
        <img
          src={imageOf(tier)}
          alt={altOf(tier, `${tier.name} — ${currency(price)} ${tier.unit ?? ""}`.trim())}
          loading="lazy"
          decoding="async"
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        ...
        <p className="mt-1 text-sm">
          <span className="font-display font-semibold text-primary">{currency(price)}</span>{" "}
        ...
        {tier.cta_type === "cart" ? (
          <button
            type="button"
            onClick={() => {
              add({ id: `plan-${tier.id}`, name: `${tier.name} plan`, price });
              toast.success(`${tier.name} added to your order`);
            }}
            className="label-caps mt-5 inline-flex min-h-[48px] items-center justify-center rounded-full bg-primary px-5 text-primary-foreground transition-all duration-200 ease-out hover:bg-primary-deep hover:scale-[1.02] active:scale-[0.97]"
          >
            Order plan
          </button>
        ) : (
